...
func _on_dialogue_processed(speaker, _dialogue, _options):
	if (
		(speaker is String and speaker == "Greg")
		or (speaker is Character and speaker.name == "Greg")
	):
...
func _on_dialogue_signal(value):
	if value == "doodle_in":
		...
		doodle_tween.tween_property(
			binkle, "position", Vector3(binkle.position.x, 0, binkle.position.z), 3.0
		)
	if value == "doodle_out":
		var doodle_tween = create_tween()
		doodle_tween.tween_property(
			binkle, "position", Vector3(binkle.position.x, -2, binkle.position.z), 3.0
		)
